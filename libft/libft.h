...
/*
 * Free all strings in strig tab and the tab pointer itself.
 */
void			free_split(char **splited);
/*
 * Allocates with `malloc(3)` and return a string that is
 * the concatenation of all strings in the tab `splited`.
 */
char			*ft_splitjoin(char **splited);
/*
 * Allocates with `malloc(3)` and return a string that is
 * the concatenation of all strings in the tab `splited` separated
 * by `sep`.
 */
